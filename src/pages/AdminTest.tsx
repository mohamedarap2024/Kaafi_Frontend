...
const AdminTest = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Product created (test)", description: "The product was added successfully." });
      setName("");
      setDescription("");
      setPrice(0);
      setCategory("");
  ...
  return (
    <div className="max-w-2xl mx-auto py-12">
      <h1 className="text-2xl font-bold mb-6">Admin Test — Add Product (unguarded)</h1>
      <form onSubmit={handleCreate} className="space-y-4 bg-card p-6 rounded-lg">
        ...
        <Button type="submit" disabled={submitting} className="w-full">
          {submitting ? "Creating..." : "Create Product (test)"}
        </Button>
      </form>
    </div>
  );
